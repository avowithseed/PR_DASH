...
// identifier는 채널 ID(UC로 시작) 또는 @handle(예: @minjoodang_tv) 둘 다 받습니다.
export async function fetchChannelSummary(identifier) {
  const isHandle = identifier.startsWith("@");
  const json = await ytGet("channels", {
    part: "snippet,statistics,contentDetails",
    ...(isHandle ? { forHandle: identifier } : { id: identifier }),
  });
  const item = json.items?.[0];
  if (!item) throw new Error(`채널을 찾을 수 없습니다: ${identifier}`);
